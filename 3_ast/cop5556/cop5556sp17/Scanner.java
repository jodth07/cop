...
public class Scanner {
	...
	public enum Kind {
		...
		RBRACE("}"), ARROW("->"), BARARROW("|->"), OR("|"),
		EQUAL("=="), NOTEQUAL("!="), LT("<"), GT(">"), LE("<="), GE(">="),

		TIMES("*"), DIV("/"), AND("&"), // strongOp
		PLUS("+"), MINUS("-"), MOD("%"), NOT("!"),
		...
		KW_SCREENHEIGHT("screenheight"), KW_SCREENWIDTH("screenwidth"), KW_XLOC("xloc"), KW_YLOC("yloc"),
		...
		private static boolean isIdentifierStart(char character) {  // Start State
		...
		static boolean canCombine(String text, Character character) { // Start state and next state
	...
	public Scanner scan() throws IllegalCharException, IllegalNumberException {
		...
		for (int i = 0; i < charsArray.length; i++) {
			...
			if (!inComment){
				...
				} else if (!Kind.canCombine(tempString, nextChar)) {
